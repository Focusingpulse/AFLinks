...
- **Claim status record (context, not evidence):** `synthesis/claim-status-records/schauberger-vortex-repulsine-1951.json` — Schauberger's vortex/implosion lineage, status *died* (suppression, contractual gag, prototype failure). The certificate documents that his *machines* were never independently tested. **Suppression is not evidence.** This card tests one small tool claim on its own merits and inherits no verdict from that history.
