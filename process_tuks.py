#!/usr/bin/env python3
"""
Phase 2: Process tuks.nl files - download, extract text, build index entries.
Time-budgeted: runs for ~105 seconds, saves progress, exits.
Run repeatedly via cron until all files are done.
"""
import os, re, json, time, tempfile, subprocess, urllib.parse, urllib.request, sys
from datetime import datetime

FILELIST = "D:/rex_archive/tuks_filelist.json"
OUTPUT = "D:/rex_archive/tuks_entries.json"
PROGRESS = "D:/rex_archive/tuks_progress.json"
INDEX_PATH = "D:/rex_archive/index.json"
BUDGET = 105  # seconds
TESSERACT = r'C:\Users\focus\AppData\Local\Programs\Tesseract-OCR\tesseract.exe'
...
def ocr_pdf(pdf_path, max_chars=2000):
    try:
        import pypdfium2 as pdfium
...
def extract_patents(text):
    ...
    for m in re.finditer(r'\bpatent\s*#?\s*(\d{6,8})\b', text, re.I):
        ...
        if f"US{p}" not in patents:
            patents.append(p)
    # Also look for patent numbers in filenames like US4622510
...
def extract_persons(filename, title):
    ...
    name_patterns = [
        ...
        (r'(Claus Turtur|Claus Turtur)', 'Claus Turtur'),
...
def main():
    ...
    for i in range(start_idx, len(filelist)):
        ...
        is_wiki = entry.get("is_wiki", False)
