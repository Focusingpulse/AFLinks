...
description: "FR->EN translation. Rigorous study of a light scalar field coupled to matter in galaxies - an alternative to dark matter proposing a scalar field mechanism for explaining galactic rotation curves."
---
...
- Spitzer photometry in the 3.6 μm band (optimal tracer of M_⋆)
...
2. **Correlation analysis:** Search for relations m(M_⋆), α(morphological type)
...
def compute_rotation_curve(radii, m, alpha, rho_0, R_d, M_total):
    ...
    for r in radii:
        ...
        # In reality, one should integrate the density profile
...
def fit_galaxy_parameters(galaxy_data, initial_guess=None):
    ...
    Returns:
        result: OptimizeResult object with optimal parameters
    """
...
def analyze_galaxy_sample(sample_data):
    """
    Analysis of a galaxy sample
    
    Args:
        sample_data: list of galaxy_data dictionaries
    ...
    for galaxy_data in sample_data:
        print(f"Fitting {galaxy_data['name']}...")
        result = fit_galaxy_parameters(galaxy_data)
        
        results['galaxies'].append(galaxy_data['name'])
        results['m_values'].append(result.x[0])
        results['alpha_values'].append(result.x[1])
        results['chi2_values'].append(result.fun)
        
        # Compton wavelength in kpc
        m_kg = result.x[0]
        lambda_c = (1.054571817e-34) / (m_kg * c) / kpc_to_m
        results['lambda_compton_kpc'].append(lambda_c)
...
### 9.1 Local Constraints

The model must satisfy several local (Solar System) constraints:
- Perihelion precession of Mercury
- Lunar laser ranging measurements
- Strong equivalence principle tests

### 9.2 Cosmological Constraints

The scalar field must be compatible with:
...
The model predicts modified lensing signatures that can be tested against observations.

---

## 10. Distinctive Predictions and Comparisons

### 10.1 Universal Parameters

A key prediction is the universality of m and α across all galaxies, unlike dark matter models where halo parameters vary.

### 10.2 Mass-Velocity Relation

The model predicts a specific Tully-Fisher-like relation emerging from the scalar field dynamics.

### 10.3 Testable Differences from MOND
...
The model offers a concrete realization of modified gravity without introducing new particles or ad hoc dynamics.

### 11.2 Experimental Tests

Future observations that could falsify the model:
- Precision rotation curves at large radii
...
This work presents a rigorous theoretical framework for a light scalar field coupled to baryonic matter as an alternative to dark matter. The model:
- Provides quantitative predictions for galactic rotation curves
- Uses physically motivated parameters (m ~ 10⁻²⁷ eV/c², α ~ G)
- Offers testable and falsifiable predictions
- Represents a concrete alternative to both dark matter and MOND

The framework is ready for systematic testing against SPARC database observations.
...
1. Rubin, V. C., & Ford, W. K. (1970). Astrophysical Journal, 159, 379.
2. Milgrom, M. (1983). Astrophysical Journal, 270, 365.
3. Lelli, F., McGaugh, S. S., & Schombert, J. M. (2016). AJ, 152, 157.
4. Brans, C., & Dicke, R. H. (1961). Physical Review, 124, 925.

### Technical Appendices

Detailed mathematical derivations, complete numerical code, and fitting results for individual galaxies are available in the supplementary materials.

---

*Translated from French by automated translation system. Original paper by Fabien Toulgoat, deposited on Zenodo (DOI: 10.5281/zenodo.16728704).*
